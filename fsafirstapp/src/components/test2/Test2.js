...
function Test2(){
    ...
    return(
        ...
        {
            skills.map(skill=><h1 key={skill}>{skill}</h1>)
        }

        <button className="btn btn-success" onClick={addNewSkill}>Add new Skill</button>
        <button className="btn btn-danger" onClick={deleteSkill}>Delete Skill</button>
        <button className="btn btn-warning" onClick={modifySkill}>Modify Skill</button>
